...
final class Base58
{
    ...
    public static function decode(string $s): string
    {
        ...
        for ($i = 0, $n = strlen($s); $i < $n; $i++) {
            $v = strpos(self::A, $s[$i]);
            if ($v === false) { throw new InvalidArgumentException("bad base58 char {$s[$i]}"); }
            $carry = $v;
            ...
            while ($carry > 0) { array_unshift($bytes, $carry & 0xFF); $carry >>= 8; }
        }
        // every leading '1' encodes one leading zero byte
        for ($i = 0; $i < strlen($s) && $s[$i] === '1'; $i++) { array_unshift($bytes, 0); }
    ...
    public static function encode(string $bin): string
    {
        ...
        while ($start < count($bytes)) {
            ...
            while ($start < count($bytes) && $bytes[$start] === 0) { $start++; }
        }
